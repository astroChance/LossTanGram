...
class LossTanGram():
    ...
    def __init__(self, input_arr, horizon_filepath, segy_filepath=None):
    ...
    #---------------------------------------------------------
    @classmethod
    def from_segy(cls, segy_filepath, horizon_filepath):
        """
        ...
        return cls(input_arr, horizon_filepath, segy_filepath)
    ...
    #---------------------------------------------------------
    def loss_tangent_segy(self, data_type='rms', filter_size=5, axis=-1, use_bias=True,
        ...
        if len(arr.shape)==2:
            for i in range(arr.shape[0]):
                if np.isnan(surface_amplitude_values[i]):
                    ...
                    time_diff = ((Z_samples)/10000000)-((Z_idxs[i]*0.375)/10000000)
    ...
    def plot_data(self, data='input', arr=None,
        ...
        if data.lower() == 'input':
            ...
            arr = self.spectral_derivative(self._input_arr)
        ...
        # Currently relies on _segy implementation
        surface_amplitude_values, Z_idxs = self.get_surface_values_segy(arr=arr, 
                                                                        delimiter=delimiter)
